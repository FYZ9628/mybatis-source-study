...
import com.ncf.mybatis.cache.decorators.FifoCache;
import com.ncf.mybatis.cache.impl.PerpetualCache;
import com.ncf.mybatis.executor.keygen.KeyGenerator;
import com.ncf.mybatis.mapping.CacheBuilder;
import com.ncf.mybatis.mapping.MappedStatement;
import com.ncf.mybatis.mapping.ResultFlag;
...
public class MapperBuilderAssistant extends BaseBuilder {
    ...
    public MappedStatement addMappedStatement (
        ...
        MappedStatement.Builder statamentBuilder = new MappedStatement.Builder(configuration, id, sqlCommandType, sqlSource, resultType);
        statamentBuilder.resource(resource);
        statamentBuilder.keyGenerator(keyGenerator);
        statamentBuilder.keyProperty(keyProperty);

        // 结果映射，给 MappedStatement#resultMaps
        setStatementResultMap(resultMap, resultType, statamentBuilder);
        setStatementCache(isSelect, flushCache, useCache, currentCache, statamentBuilder);

        MappedStatement statement = statamentBuilder.build();
        // 映射语句信息，建造完存放到配置项中
    ...
    private void setStatementCache(
            ...
            boolean useCache,
            Cache cache,
            MappedStatement.Builder statamentBuilder) {
        flushCache = valueOrDefault(flushCache, !isSelect);
        useCache = valueOrDefault(useCache, isSelect);
        statamentBuilder.flushCacheRequired(flushCache);
        statamentBuilder.useCache(useCache);
        statamentBuilder.cache(cache);
    }
